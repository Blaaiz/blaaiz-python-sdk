...
class CustomerService:
    ...
    def create(self, customer_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        required_fields = ['first_name', 'last_name', 'type', 'email', 'country', 'id_type', 'id_number']
    ...
    def add_kyc(self, customer_id: str, kyc_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return self.client.make_request('POST', f'/api/external/customer/{customer_id}/kyc-data', kyc_data)
    ...
    def upload_files(self, customer_id: str, file_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return self.client.make_request('PUT', f'/api/external/customer/{customer_id}/files', file_data)
    
    def upload_file_complete(self, customer_id: str, file_options: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if file_category not in ['identity', 'proof_of_address', 'liveness_check']:
            raise ValueError("file_category must be one of: identity, proof_of_address, liveness_check")
        
        try:
            # Step 1: Get presigned URL
            presigned_response = self.client.make_request('POST', '/api/external/file/get-presigned-url', {
            ...
            # Step 3: Upload to S3
            self._upload_to_s3(presigned_url, file_buffer['data'], file_buffer.get('content_type'), file_buffer.get('filename'))
            
            # Step 4: Associate file with customer
            file_association = self.client.make_request('PUT', f'/api/external/customer/{customer_id}/files', {
                'id_file': file_id
            })
    ...
    def _process_file_content(self, file_content: Union[bytes, str], content_type: Optional[str], filename: Optional[str]) -> Dict[str, Any]:
    ...
    def _upload_to_s3(self, presigned_url: str, file_data: bytes, content_type: Optional[str], filename: Optional[str]):
